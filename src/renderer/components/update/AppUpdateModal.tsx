...
const AppUpdateModal: React.FC<AppUpdateModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center modal-backdrop" onClick={handleBackdropClick}>
      <div
      ...
      >
        ...
        {modalState === 'info' && (
          <>
            <div className="px-5 pt-5 pb-4">
              ...
              {currentLog.title && (
                <p className="mt-3 text-sm font-medium dark:text-claude-darkText text-claude-text">{currentLog.title}</p>
              )}
        ...
        {modalState === 'downloading' && (
          <div className="px-5 py-5">
            <h3 className="text-base font-semibold dark:text-claude-darkText text-claude-text">{i18nService.t('updateDownloading')}</h3>
            <p className="mt-1.5 text-xs dark:text-claude-darkTextSecondary text-claude-textSecondary">v{latestVersion}</p>
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 flex items-center justify-between text-xs dark:text-claude-darkTextSecondary text-claude-textSecondary">
                ...
                <span className="flex items-center gap-3">
                  {downloadProgress?.speed != null && <span>{formatSpeed(downloadProgress.speed)}</span>}
                  {downloadProgress?.percent != null && <span>{Math.round(downloadProgress.percent * 100)}%</span>}
                </span>
              </div>
            </div>
        ...
        {modalState === 'installing' && (
          <div className="px-5 py-5">
            <div className="flex flex-col items-center py-4">
              <svg className="animate-spin h-8 w-8 text-claude-accent" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        {modalState === 'error' && (
          <div className="px-5 py-5">
            ...
            {errorMessage && (
              <p className="mt-2 text-sm dark:text-claude-darkTextSecondary text-claude-textSecondary break-words">{errorMessage}</p>
            )}
