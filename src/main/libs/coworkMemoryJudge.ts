/**
 * 这个文件负责对“已经抽取出来的记忆候选”做最终接纳判定。
 *
 * 主要职责：
 * 1. 先用规则对候选文本打分，区分稳定个人信息、偏好、临时上下文和流程性内容。
 * 2. 根据显式/隐式类型和 guard level 计算不同的接纳阈值。
 * 3. 对接近阈值的边界样本，可选调用 LLM 做一次低成本复核。
 * 4. 通过缓存减少重复 LLM 判定，最终输出统一的 `MemoryJudgeResult`。
 */
...
// 规则分数距离阈值在这个范围内时，视为边界案例，可选交给 LLM 复核。
const LLM_BORDERLINE_MARGIN = 0.08

// LLM 返回结果低于这个置信度时，放弃采用。
...
// 判断是否属于“规则分接近阈值”的边界案例，需要调用 LLM 二次判定。
...
// 将基础地址规范化为 Anthropic `/v1/messages` 接口地址。
...
// 从 Anthropic 风格响应中提取纯文本内容，兼容数组块和直接文本两种结构。
...
// 解析 LLM 返回的 JSON 判定结果；兼容 fenced code block 和松散字段名。
...
async function judgeWithLlm(
  ...
  // 用极短提示词把任务约束成“稳定长期记忆”二分类，避免模型自由发挥。
  const systemPrompt = [
...
// 对记忆候选进行最终判定：规则优先，边界场景可选 LLM 复核。
