...
function persistSandboxSpawnDiagnostics(runtimeInfo: SandboxRuntimeInfo, details: string): string | null {
  try {
...
function formatSandboxSpawnError(error: unknown, runtimeInfo: SandboxRuntimeInfo): string {
  ...
  const err = error && typeof error === 'object' ? (error as NodeJS.ErrnoException & { spawnargs?: string[] }) : null
  ...
  const hint = err?.code === 'ENOEXEC' || err?.errno === -8 ? ' Possible exec format mismatch (wrong arch or compressed binary).' : ''
...
interface ActiveSession {
  ...
  // 确认模式：'modal'表示需要弹窗确认，'text'表示用户需要输入特定文本确认
  confirmationMode: 'modal' | 'text'
  // 在等待用户批准工具使用权限时的相关信息；如果不为 null，表示当前正在等待用户批准某个工具的使用权限
  pendingPermission: PermissionRequest | null
  // 用于取消当前会话的 AbortController
...
export class CoworkRunner extends EventEmitter {
  ...
  private runConversationSearchTool(args: { query: string; max_results?: number; before?: string; after?: string }): string {
  ...
  private runRecentChatsTool(args: { n?: number; sort_order?: 'asc' | 'desc'; before?: string; after?: string }): string {
  ...
  private runMemoryUserEditsTool(args: {
    ...
    if (args.action === 'list') {
      ...
      const payload =
        entries.length === 0
          ? 'memories=(empty)'
          : entries.map((entry) => `${entry.id} | ${entry.status} | explicit=${entry.isExplicit ? 1 : 0} | ${entry.text}`).join('\n')
      return {
  ...
  private collectHostSkillsRoots(env: Record<string, string | undefined>, cwdMapping: SandboxCwdMapping, systemPrompt: string): string[] {
  ...
  private collectSandboxSkillEntries(hostSkillsRoots: string[], guestSkillsRoot: string): SandboxSkillEntry[] {
  ...
  private resolveSandboxSkillsConfig(
    ...
    const keyOf = (target: string): string => (process.platform === 'win32' ? target.toLowerCase() : target)
  ...
  private buildSandboxEnv(env: Record<string, string | undefined>, guestSkillsRoot: string | null): Record<string, string> {
    ...
    const remapLocalhostToQemuGateway = (url: string): string => {
      return url.replace(/\/\/localhost([:/])/gi, '//10.0.2.2$1').replace(/\/\/127\.0\.0\.1([:/])/g, '//10.0.2.2$1')
    }
    ...
    const shouldForceDirectHostRouting =
      anthropicBaseHost === '10.0.2.2' || anthropicBaseHost === '127.0.0.1' || anthropicBaseHost === 'localhost'
  ...
  private stageExternalAttachment(cwd: string, sourcePath: string, sessionId: string, index: number): string | null {
   ...
   */
  private pushStagedAttachmentsToSandbox(bridge: VirtioSerialBridge, cwd: string, sessionId: string): void {
    const stageRoot = path.join(cwd, SANDBOX_ATTACHMENT_DIR, sessionId)
    ...
    for (const file of files) {
      bridge.pushFile(SANDBOX_WORKSPACE_GUEST_ROOT, `${guestAttachmentDir}/${file.relativePath}`, file.data)
    }
  ...
  private shouldEmitStreamingUpdate(lastEmitAt: number, force = false): { emit: boolean; now: number } {
    const now = Date.now()
  ...
  private buildHistoryBlocks(
    ...
    if (trimmedCurrentPrompt && last?.type === 'user' && last.content.trim() === trimmedCurrentPrompt) {
      history.pop()
    }
  ...
  private rewriteSkillPathsForSandbox(content: string, skillPath: string, options: SandboxSkillRewriteOptions): string {
    ...
    for (const mapping of mappings) {
      const sourceVariants = new Set<string>([mapping.hostRoot, mapping.hostRoot.replace(/\\/g, '/')])
  ...
  private rewriteSkillLocationForSandbox(skillLocation: string, options: SandboxSkillRewriteOptions): string | null {
    const guestSkillsRoot = options.guestSkillsRoot?.trim()
    if (!guestSkillsRoot) {
      return null
    }
  ...
  private rewriteSkillReferencesForSandbox(
    ...
    for (const mapping of this.buildSandboxSkillRootMappings(options)) {
      const variants = new Set<string>([mapping.hostRoot, mapping.hostRoot.replace(/\\/g, '/')])
      let next = rewritten
  ...
  private buildSandboxSkillRootMappings(options: SandboxSkillRewriteOptions): Array<{ hostRoot: string; guestRoot: string }> {
  ...
  private mapHostSkillPathToSandboxPath(hostPath: string, options: SandboxSkillRewriteOptions): string | null {
  ...
  private buildWorkspaceSafetyPrompt(workspaceRoot: string, cwd: string, confirmationMode: 'modal' | 'text'): string {
  ...
  private async ensureWindowsPythonRuntimeForCommand(sessionId: string, command: string): Promise<{ ok: boolean; reason?: string }> {
    ...
    const reason =
      runtimeResult.error || (isPipCommand ? 'Bundled Python pip environment is unavailable.' : 'Bundled Python runtime is unavailable.')
  ...
  async startSession(
    ...
    if (session.cwd !== sessionCwd) {
      // 若解析后的 cwd 与存储值不同，回写到会话记录。
      this.store.updateSession(sessionId, { cwd: sessionCwd })
    }

    // 组合最终系统提示词（安全策略、记忆策略、平台策略 + 业务 prompt）。
  ...
  private handleHostToolExecution(payload: Record<string, unknown>): { success: boolean; text: string } {
    ...
    const toolInput = rawInput && typeof rawInput === 'object' ? (rawInput as Record<string, unknown>) : {}
    ...
    try {
      ...
      if (toolName === 'recent_chats') {
        const sortOrder = toolInput.sort_order === 'asc' || toolInput.sort_order === 'desc' ? toolInput.sort_order : undefined
      ...
      if (toolName === 'memory_user_edits') {
        ...
        const result = this.runMemoryUserEditsTool({
          ...
          status:
            toolInput.status === 'created' || toolInput.status === 'stale' || toolInput.status === 'deleted' ? toolInput.status : undefined,
  ...
  private async runClaudeCodeLocal(
    ...
    // 读取运行配置（用于是否注入记忆工具等行为开关）。
    const config = this.store.getConfig()

    // 入口兜底：若会话已停止/中止，直接结束并清理挂起权限。
    ...
    // 重置“本轮输出去重/截断”状态，避免上一轮流式状态污染当前轮。
    ...
    // 输出 MCP 相关环境诊断日志，便于排查打包后路径/环境变量问题。
    ...
    // 打包态下 process.execPath 指向 Electron 可执行文件。
    // SDK 若走默认 fork，会复用该路径；这里强制开启 Node 模式，避免误拉起新应用实例。
    if (app.isPackaged) {
      envVars.ELECTRON_RUN_AS_NODE = '1'
    }

    // Windows 本地模式依赖 Git Bash 执行 shell 工具，启动前先做健康检查。
    if (process.platform === 'win32' && !envVars.CLAUDE_CODE_GIT_BASH_PATH) {
      const bashResolutionDiagnostic =
        typeof envVars.LOBSTERAI_GIT_BASH_RESOLUTION_ERROR === 'string' ? envVars.LOBSTERAI_GIT_BASH_RESOLUTION_ERROR.trim() : ''
      const errorMsg =
    ...
    const handleSdkStderr = (message: string): void => {
      // 保留 stderr 尾部窗口，避免无限增长，同时用于失败回传。
      stderrTail += message
    ...
    const options: Record<string, unknown> = {
      ...
      canUseTool: async (toolName: string, toolInput: unknown, { signal }: { signal: AbortSignal }): Promise<PermissionResult> => {
        // 工具调用前的统一中止判断，避免在已终止会话中继续执行工具。
        ...
        const resolvedInput = toolInput && typeof toolInput === 'object' ? (toolInput as Record<string, unknown>) : { value: toolInput }

        // Windows 下对 Bash 中的 Python/pip 命令做内置运行时可用性拦截。
        ...
        // 普通工具默认放行，AskUserQuestion 走人工回答回填流程。
        if (resolvedName !== 'AskUserQuestion') {
          return { behavior: 'allow', updatedInput: resolvedInput }
        }
        ...
        // AskUserQuestion 必须包含 answers，缺失则视为无效输入并拒绝。
    ...
    // if (app.isPackaged) {
    // 打包态覆盖 SDK 进程拉起器：统一强制 Node 模式并处理平台差异。
    options.spawnClaudeCodeProcess = (spawnOptions: {
      ...
      const useElectronShim = process.platform === 'win32' || spawnOptions.env?.LOBSTERAI_NODE_SHIM_ACTIVE === '1'
      ...
      // Windows 下若 SDK 传入 node/node.cmd，改写到内置 Electron Node 运行时。
      ...
      // macOS 打包态下若命令指向应用自身，改写为 helper runtime，避免自启动死循环。
      ...
      // Windows 可选注入隐藏子进程预加载，减少闪烁窗口与干扰。
    ...
    // 本地 SDK 会话与子进程强绑定；stop 后不可可靠恢复。
    // 因此这里清空 sessionId，改为在重启时通过提示词注入历史来续接上下文。
    ...
    try {
      ...
      const memoryTools: any[] = [
        tool(
          ...
          async (args: { query: string; max_results?: number; before?: string; after?: string }) => {
        ...
        tool(
          ...
          async (args: { n?: number; sort_order?: 'asc' | 'desc'; before?: string; after?: string }) => {
      ...
      if (this.mcpServerProvider) {
        try {
          ...
          for (const server of enabledMcpServers) {
            ...
            // 与已注入服务（如 memory server）同名时跳过，避免覆盖。
      ...
      if (options.mcpServers) {
        ...
        // 导出完整 MCP 配置 JSON（SDK 实例对象使用占位说明）。
        try {
          const serializable: Record<string, unknown> = {}
          for (const key of mcpKeys) {
            const cfg = (options.mcpServers as Record<string, Record<string, unknown>>)[key]
            if (cfg && typeof cfg === 'object') {
              // 仅序列化普通配置对象；SDK 实例不可直接序列化。
      ...
      // 组装 prompt：存在图片时改为多模态 message block；否则沿用纯文本。
      ...
      if (imageAttachments && imageAttachments.length > 0) {
        ...
        // 构造一次性 async iterable，兼容 SDK 的流式输入接口。
      ...
      // 统一清理权限挂起与 activeSession 映射，避免状态泄漏到下一轮。
      this.clearPendingPermissions(sessionId)
      this.activeSessions.delete(sessionId)
    }
  }
  ...
  private async runClaudeCode(
    ...
    // 入口兜底：若会话已被用户主动停止或已中止，则立即结束本轮并清理状态。
    if (this.isSessionStopRequested(sessionId, activeSession)) {
      this.store.updateSession(sessionId, { status: 'idle' })
      this.clearPendingPermissions(sessionId)
      this.activeSessions.delete(sessionId)
    ...
    if (shouldPrepareSandboxPrompt) {
      // 沙箱预处理会把可映射的外部附件改写为沙箱可访问路径；
      // 同时返回无法映射的附件列表供后续统一拦截。
    ...
    // 汇总“工作目录外”的附件路径（包括解析失败导致无法映射的路径）。
    const outsideAttachments = Array.from(
      new Set([...this.findAttachmentsOutsideCwd(effectivePrompt, resolvedCwd), ...unresolvedSandboxAttachments])
    )

    // 判断是否存在可复用的沙箱 VM：会话模式为 sandbox、进程未退出、IPC 桥可用。
    ...
    // 当附件越界且当前会走/正在走沙箱时，执行安全分支：
    // 1) 强制沙箱模式：直接报错；2) 自动模式且未锁定沙箱：回退本地执行。
    ...
    // 复用已有沙箱：直接走“续轮”请求，避免重复拉起 VM，提升多轮对话稳定性与性能。
    ...
    if (!sandboxReady.ok) {
      ...
      // 用户强制 sandbox 模式时，不允许静默回退，直接失败并结束本轮。
      ...
      // auto/其他可回退模式：必要时提示原因，然后切本地继续执行。
    ...
    try {
      // 首次进入沙箱或沙箱重启后，注入裁剪后的历史上下文，维持连续对话体验。
      ...
      // 沙箱执行结束后：
      // - VM 仍存活：保留 activeSession，供后续继续复用；
      // - VM 已退出：清理活跃会话映射，避免悬挂状态。
      ...
      // 强制 sandbox 模式下，沙箱错误不可回退，直接按失败处理。
      if (executionMode === 'sandbox') {
  ...
  private async runClaudeCodeInSandbox(
    ...
    activeSession.sandboxSkillMounts = Object.keys(sandboxSkills.skillMounts).length > 0 ? sandboxSkills.skillMounts : undefined
    activeSession.sandboxSkillRootMounts = sandboxSkills.rootMounts.length > 0 ? sandboxSkills.rootMounts : undefined
    ...
    const isWhpxFailed = (message: string) => /WHPX|whpx/.test(message) && /fail|error|not.*support|unavailable/i.test(message)
    const isMemoryAllocationFailed = (message: string) => message.includes('cannot set up guest memory')
    ...
    const runOnce = async (
      ...
      console.log(
        `Starting sandbox VM with acceleration: ${accelMode}, launcher: ${launcherOverride ?? 'direct'}, memory: ${memoryMb ?? 4096}MB`
      )
      ...
      const handleLine = (line: string) => {
        ...
        if (messageType === 'permission_request') {
          ...
          const toolInput = toolInputRaw && typeof toolInputRaw === 'object' ? (toolInputRaw as Record<string, unknown>) : {}
      ...
      try {
        ...
        const defaultVmReadyTimeout = runtimeInfo.platform === 'win32' && accelMode === 'tcg' ? 300000 : 180000
        const vmReadyTimeoutMs =
          Number.isFinite(vmReadyTimeoutOverride) && vmReadyTimeoutOverride > 0 ? vmReadyTimeoutOverride : defaultVmReadyTimeout
  ...
  private resolveAutoRoutingForSandbox(systemPrompt: string, options: SandboxSkillRewriteOptions = {}): string {
    const guestSkillsRoot = options.guestSkillsRoot?.trim()
    ...
    if (guestSkillsRoot) {
      ...
      const rewritten = rewrittenPrompt.replace(/<location>(.*?)<\/location>/g, (_fullMatch: string, rawLocation: string) => {
        const mapped = this.rewriteSkillLocationForSandbox(rawLocation, options)
        if (!mapped) {
  ...
  private handleClaudeEvent(sessionId: string, event: unknown): void {
    ...
    // 这类事件会以 start/delta/stop 的形式连续到达，
    // 统一交给 handleStreamEvent 做“增量写入 + 节流更新 + 收尾落盘”，
    // 避免在本函数里混入过多状态机细节。
    ...
    if (eventType === 'result') {
      // 记录 token 用量，便于后续观测和排障。
      // 注意：不同 SDK 版本可能把 usage 放在 payload.usage 或 payload.result.usage。
      const usage = (payload.usage ??
        (payload.result && typeof payload.result === 'object' ? (payload.result as Record<string, unknown>).usage : undefined)) as
        | Record<string, unknown>
        | undefined
      if (usage) {
      ...
      // 沙箱模式下：收到成功结果后将本轮标记完成。
      // 这里只结束当前 turn，不销毁 VM，以便后续多轮复用。
      // 也就是说“会话完成”是逻辑完成，不等于“沙箱进程退出”。
      if (activeSession.executionMode === 'sandbox') {
        ...
        // 通知本轮结束，供等待方继续推进后续逻辑。
        // runClaudeCodeInSandbox / continueSandboxTurn 都依赖这个 resolve 解除等待。
        if (activeSession.sandboxTurnResolve) {
    ...
    if (eventType === 'user') {
      // 某些 SDK/tool 场景下，工具结果会通过 user 事件回传。
      // 这里将其中的 tool_result 显式转成消息，保证前端时间线完整可见。
      const messagePayload = payload.message
      if (!messagePayload || typeof messagePayload !== 'object') {
        return
      }
    ...
    // 判断是否已从流式分支产出 assistant 文本/思考内容。
    // 这里用 hasAssistant* 标记而非当前流式缓存状态，
    // 因为 content_block_stop 可能已经把流式状态清空。
    // 若仍按“当前缓存是否为空”判断，会误判成“没输出过”，导致重复落消息。
    ...
    // 在走 assistant 回退解析前，先把残留流式缓存落盘。
    // 这是为了兜底处理“assistant 事件先到、stream stop 后到”这类乱序场景：
    // 先 finalize 才能保证 delta 缓冲不会被后续回退逻辑覆盖或遗失。
    ...
    const messagePayload = payload.message
    if (!messagePayload || typeof messagePayload !== 'object') {
      // message 结构异常或缺失时，尝试 extractText 做兜底。
      // 但如果本轮已经有流式文本，则必须跳过这个兜底分支，避免重复消息。
    ...
    if (!Array.isArray(contentBlocks)) {
      // content 不是标准数组时走扁平提取兜底。
      // 同样遵守“流式优先”原则：有流式则不再追加回退文本。
      if (hasStreamedText || hadPendingTextStreaming) return
    ...
    const flushTextParts = () => {
      // 将暂存的 textParts 刷新为一条 assistant 消息。
      // 触发时机：遇到非 text 块（如 thinking/tool_use/tool_result）或循环结束。
      // 仍需做双重保护：
      // 1) 已有流式输出则不刷，防止重复；
      // 2) 空分片不刷，防止产生空消息。
    ...
    for (const block of contentBlocks) {
      ...
      if (blockType === 'thinking' && typeof record.thinking === 'string' && record.thinking.trim()) {
        // thinking 与 text 分开存储，便于前端区分“思考中间态”和“最终回答”。
        // 如果流式阶段已经产出过 thinking，这里跳过，避免重复。
      ...
      if (blockType === 'text' && typeof record.text === 'string') {
        // 文本块先拼接，延迟到合适时机一次性入库，减少碎片消息。
        textParts.push(record.text)
        continue
      }

      if (blockType === 'tool_use') {
        // 进入工具调用块前先 flush 文本，保证消息顺序为：文本 -> 工具调用。
        // 这样前端展示顺序更符合模型真实执行轨迹。
        flushTextParts()
        const toolName = String(record.name ?? 'unknown')
        const toolInputRaw = record.input ?? {}
        const toolInput =
          toolInputRaw && typeof toolInputRaw === 'object' ? (toolInputRaw as Record<string, unknown>) : { value: toolInputRaw }
      ...
      if (blockType === 'tool_result') {
        // 工具结果同理，先刷掉待发送文本，再落 tool_result。
        // 这样能避免“工具结果插入到上一段文本中间”的时间线错位。
        flushTextParts()
        const content = this.formatToolResultContent(record)
        const isError = Boolean(record.is_error)
        const message = this.store.addMessage(sessionId, {
    ...
    // 循环结束后做最后一次刷新，确保末尾文本不丢失。
    flushTextParts()
  }

  private handleStreamEvent(sessionId: string, activeSession: ActiveSession, payload: Record<string, unknown>): void {
    // SDKPartialAssistantMessage 的结构示意：
    ...
    // 处理 content_block_start：初始化一段新的流式消息。
    ...
    // 处理 content_block_delta：对当前流式消息做增量更新。
    ...
    // 处理 content_block_stop：结束当前 block 并完成落盘。
    if (eventType === 'content_block_stop') {
      const blockType = activeSession.currentStreamingBlockType

      if (blockType === 'thinking') {
        // 收尾 thinking 消息：写入最终内容并清理状态。
    ...
    // 处理 message_stop：兜底确保所有流式内容都被最终落盘。
    if (eventType === 'message_stop') {
      // 收尾所有未完成的 thinking 消息。
      if (activeSession.currentStreamingThinkingMessageId && activeSession.currentStreamingThinking) {
        this.updateMessageMerged(sessionId, activeSession.currentStreamingThinkingMessageId, {
          content: activeSession.currentStreamingThinking,
          metadata: { isStreaming: false }
  ...
  private waitForPermissionResponse(sessionId: string, requestId: string, signal?: AbortSignal): Promise<PermissionResult> {
  ...
  private async waitForVmReady(
    ...
    const defaultMaxTimeout = options?.platform === 'win32' ? Math.max(timeout, options?.accelMode === 'tcg' ? 900000 : 420000) : timeout
      ...
      while (Date.now() - start < currentTimeoutMs) {
        ...
        try {
          if (fs.existsSync(heartbeatPath)) {
            ...
            const timestamp = typeof data.timestamp === 'number' ? data.timestamp : Number.parseInt(String(data.timestamp ?? ''), 10)
  ...
  private async readSandboxStream(streamPath: string, onLine: (line: string) => void, signal: AbortSignal): Promise<void> {
  ...
  private updateMessageMerged(
    ...
    const mergedMetadata = updates.metadata ? { ...(existing?.metadata ?? {}), ...updates.metadata } : undefined
  ...
  private persistFinalResult(sessionId: string, activeSession: ActiveSession, resultText: string): void {
