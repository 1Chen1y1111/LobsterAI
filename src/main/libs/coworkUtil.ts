/**
 * 这个文件负责为 Cowork 运行时组装“可执行、可联网、可找到工具”的增强环境。
 *
 * 主要职责：
 * 1. 处理开发态和打包态下 Node、npm、npx、Git Bash、Python 等运行时发现与注入。
 * 2. 修复 Windows 上 PATH、系统变量、MSYS2、编码和 Git Bash 继承环境的问题。
 * 3. 注入 API 配置、内部代理地址、技能目录、系统代理和临时目录等运行上下文。
 * 4. 提供模型可用性探测和会话标题生成等辅助能力。
 */
...
// 合并 PATH 类变量并去重，保持传入 additions 的优先级更靠前。
...
// 在指定环境中探测某个命令是否可以被 `which/where` 找到。
...
// 缓存 Electron 作为 Node runtime 时应使用的可执行文件路径。
let cachedElectronNodeRuntimePath: string | null = null

// 解析 Electron 充当 Node 运行时时的实际可执行文件，兼容 macOS 打包 helper。
...
// 对外暴露 Electron Node runtime 路径，并做一次性缓存。
...
// 解析用户登录 shell 的 PATH，主要用于 macOS/Linux 打包态补全工具路径。
/**
...
// 从 Windows 注册表获取最新 PATH，修复 Explorer 继承旧环境导致的工具缺失。
/**
...
// 把注册表中的 PATH 条目合并进当前环境，避免新安装工具在应用里不可见。
/**
...
// 规范化 Windows 路径文本，去引号、去 CR，并统一为反斜杠。
...
// 运行 `where` 一类命令并收集存在的 Windows 可执行文件路径。
...
// 返回应用内置 PortableGit 的 bash 候选路径。
...
// 检查某个 Git Bash 是否健康可用，核心验证点是 `cygpath` 能否正常执行。
...
// 生成 node / npm / npx shim，让 Electron 可以在子进程里充当 Node 运行时。
...
// 在 Windows 上解析并选出最合适的 Git Bash 路径。
/**
...
// 确保 Windows 必需系统变量存在，避免系统命令或 DLL 定位失败。
/**
...
// 给非 login 模式的 Git Bash 预置最小可用的 MSYS 命令路径。
/**
...
// 把单个 Windows 路径转换成 MSYS/POSIX 形式，避免非 ASCII 路径在启动阶段被错误转码。
/**
...
// 把整个 Windows PATH 转成 MSYS/POSIX PATH，供 Git Bash 的 ORIGINAL_PATH 使用。
/**
...
// 预先设置 POSIX 格式的 ORIGINAL_PATH，防止 Git Bash 错误解析 Windows PATH。
/**
...
// 生成一个 Bash 初始化脚本，把 Windows 控制台代码页切到 UTF-8。
/**
...
// 按平台和打包状态补齐 Cowork 子进程运行所需的环境变量与路径。
...
// 对最终环境做一次 node / npm / npx 可执行性诊断，主要用于定位启动问题。
/**
...
// 生成供 Cowork 子进程使用的增强环境，包含 API、技能目录、代理和运行时修复。
/**
...
// 确保工作目录下存在 `.cowork-temp`，让子进程临时文件尽量落在项目目录附近。
/**
...
// 在增强环境基础上再注入 TMPDIR/TMP/TEMP，约束 SDK 的临时文件目录。
/**
...
// 把任意基础 URL 规整成 Anthropic `/v1/messages` 接口地址。
...
// 调用模型根据用户输入生成简短会话标题；失败时退回本地保底标题。
