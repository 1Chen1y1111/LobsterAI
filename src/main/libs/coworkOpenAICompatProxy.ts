...
// 统一提取 tool_call 的 extra_content/thought_signature。
...
// 从 OpenAI tool_calls 数组缓存 extra_content。
...
// 从 OpenAI 响应结构中提取并缓存 extra_content。
...
// 为请求中的 tool_calls 回填 extra_content（含 Gemini 回退签名）。
function hydrateOpenAIRequestToolCalls(body: Record<string, unknown>, provider?: string, baseURL?: string): void {
  ...
  for (const message of messages) {
    ...
    for (const toolCall of toArray(messageObj.tool_calls)) {
      ...
      if (isGemini) {
        // Gemini 工具调用依赖 thought_signature，缺失时注入回退签名。
...
function estimateTokenCountForText(text: string): number {
  ...
  // 非 Anthropic 后端不支持 count_tokens 时的启发式回退。
...
// 估算 count_tokens 接口返回的 input_tokens。
...
function convertChatCompletionsRequestToResponsesRequest(chatRequest: Record<string, unknown>): Record<string, unknown> {
  ...
  const maxOutputTokens =
    toNumber(chatRequest.max_output_tokens) ?? toNumber(chatRequest.max_completion_tokens) ?? toNumber(chatRequest.max_tokens)
...
function filterOpenAIToolsForProvider(openAIRequest: Record<string, unknown>, provider?: string): void {
  if (provider !== 'openai') {
    return
  }
  ...
  if (filteredTools.length !== tools.length) {
    ...
    if (toolChoiceObj) {
      const forcedName = normalizeToolName(toolChoiceObj.name) || normalizeToolName(toOptionalObject(toolChoiceObj.function)?.name)
      if (forcedName === 'skill') {
...
/**
 * MiniMax 仅接受 system/user/assistant 角色。
 * 将 developer 角色映射为 system 以兼容 MiniMax。
 */
function remapMessageRolesForMiniMax(openAIRequest: Record<string, unknown>, provider?: string): void {
  if (provider !== 'minimax') {
    return
  }
...
function clampMaxTokensFromError(openAIRequest: Record<string, unknown>, errorMessage: string): { changed: boolean; clampedTo?: number } {
...
function shouldUseMaxCompletionTokensForModel(model: unknown): boolean {
  ...
  return (
    resolvedModel.startsWith('gpt-5') || resolvedModel.startsWith('o1') || resolvedModel.startsWith('o3') || resolvedModel.startsWith('o4')
  )
}

function normalizeMaxTokensFieldForOpenAIProvider(openAIRequest: Record<string, unknown>, provider?: string): void {
  if (provider !== 'openai') {
    return
  }
...
/**
 * 将多个 system 消息合并为开头单条消息。
 * 兼容部分不接受多 system 的上游服务（如 MiniMax）。
 */
...
function isMaxTokensUnsupportedError(errorMessage: string): boolean {
  const normalized = errorMessage.toLowerCase()
  return normalized.includes('max_tokens') && normalized.includes('max_completion_tokens') && normalized.includes('not supported')
}
...
function isToolsUnsupportedError(errorMessage: string): boolean {
  const normalized = errorMessage.toLowerCase()
  return normalized.includes('does not support tools') || normalized.includes('tool use is not supported')
}

/**
 * 从 OpenAI 请求中移除 tools 与 tool_choice。
 * 返回值表示是否实际发生移除。
 */
...
function convertMaxTokensToMaxCompletionTokens(openAIRequest: Record<string, unknown>): { changed: boolean; convertedTo?: number } {
  const maxTokens = openAIRequest.max_tokens
  if (typeof maxTokens !== 'number' || !Number.isFinite(maxTokens)) {
    return { changed: false }
  }
...
function readRequestBody(req: http.IncomingMessage): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const decodeBody = (raw: Buffer): string => {
      ...
      // Windows 本地 shell（尤其 Git Bash/curl）可能使用系统代码页而非 UTF-8。
...
function detectResponsesFinishReason(responseObj: Record<string, unknown>): string {
  ...
  if (status === 'incomplete' && (incompleteReason === 'max_output_tokens' || incompleteReason === 'max_tokens')) {
    return 'length'
  }
  return 'stop'
}
...
function ensureMessageStart(res: http.ServerResponse, state: StreamState, chunk: OpenAIStreamChunk): void {
...
function ensureToolUseBlock(res: http.ServerResponse, state: StreamState, index: number, toolCall: ToolCallState): void {
...
function processOpenAIChunk(res: http.ServerResponse, state: StreamState, chunk: OpenAIStreamChunk): void {
  ...
  if (Array.isArray(delta?.tool_calls)) {
    for (const item of delta.tool_calls) {
      ...
      const normalizedExtraContent = normalizeToolCallExtraContent(item as unknown as Record<string, unknown>)
...
function syncToolCallStateWithResponsesFunctionCall(state: StreamState, callState: ResponsesFunctionCallState): ToolCallState {
...
function emitResponsesFunctionCallArgumentsOnce(
  ...
  const resolvedArguments = argumentsText || callState.finalArguments || callState.argumentsBuffer || '{}'
...
function emitResponsesCompletedFunctionCalls(
  ...
  for (const [index, item] of toArray(responseObj.output).entries()) {
    ...
    const finalizedArguments =
      normalizeFunctionArguments(itemObj.arguments) || callState.finalArguments || callState.argumentsBuffer || '{}'
...
function processResponsesStreamEvent(
  ...
  if (eventType === 'response.function_call_arguments.done') {
    const callState = registerResponsesFunctionCallState(context, payloadObj, null)
    const argumentsDone = normalizeFunctionArguments(payloadObj.arguments) || callState.argumentsBuffer || '{}'
...
async function handleResponsesStreamResponse(upstreamResponse: Response, res: http.ServerResponse): Promise<void> {
  res.writeHead(200, {
...
async function handleChatCompletionsStreamResponse(upstreamResponse: Response, res: http.ServerResponse): Promise<void> {
  res.writeHead(200, {
    'Content-Type': 'text/event-stream',
    'Cache-Control': 'no-cache',
    Connection: 'keep-alive'
...
async function handleCreateScheduledTask(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
...
async function handleListScheduledTasks(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  if (!scheduledTaskDeps) {
    writeJSON(res, 503, { success: false, error: 'Scheduled task service not available' } as any)
    return
  }
...
async function handleGetScheduledTask(req: http.IncomingMessage, res: http.ServerResponse, id: string): Promise<void> {
  if (!scheduledTaskDeps) {
    writeJSON(res, 503, { success: false, error: 'Scheduled task service not available' } as any)
    return
  }
...
async function handleUpdateScheduledTask(req: http.IncomingMessage, res: http.ServerResponse, id: string): Promise<void> {
  if (!scheduledTaskDeps) {
    writeJSON(res, 503, { success: false, error: 'Scheduled task service not available' } as any)
    return
  }
...
async function handleDeleteScheduledTask(req: http.IncomingMessage, res: http.ServerResponse, id: string): Promise<void> {
  if (!scheduledTaskDeps) {
    writeJSON(res, 503, { success: false, error: 'Scheduled task service not available' } as any)
    return
  }
...
async function handleToggleScheduledTask(req: http.IncomingMessage, res: http.ServerResponse, id: string): Promise<void> {
  if (!scheduledTaskDeps) {
    writeJSON(res, 503, { success: false, error: 'Scheduled task service not available' } as any)
    return
  }
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  // 先匹配更具体的 toggle 路径，再匹配通用 item 路径。
  ...
  if (itemMatch) {
    ...
    if (method === 'GET') {
      await handleGetScheduledTask(req, res, id)
      return
    }
    if (method === 'PUT') {
      await handleUpdateScheduledTask(req, res, id)
      return
    }
    if (method === 'DELETE') {
      await handleDeleteScheduledTask(req, res, id)
      return
    }
  }
  ...
  // 强制将请求模型重映射为用户配置模型。
  // 规避探测/预热阶段出现的内部模型名被非 Anthropic 上游拒绝。
  ...
  // 部分 provider（如 MiniMax）拒绝多个 system 消息，发送前统一合并。
  mergeSystemMessagesForProvider(openAIRequest)

  const upstreamRequest = upstreamAPIType === 'responses' ? convertChatCompletionsRequestToResponsesRequest(openAIRequest) : openAIRequest
  ...
  if (!upstreamResponse.ok) {
    ...
    if (!upstreamResponse.ok) {
      ...
      if (upstreamAPIType === 'chat_completions' && upstreamResponse.status === 400) {
        // 部分 Ollama 模型不支持工具调用：检测到错误后剥离 tools 并重试。
        ...
        // 部分 OpenAI 兼容服务（如 DeepSeek）严格限制 max_tokens。
        // 若错误中含范围信息，则夹逼后重试一次。
        if (!upstreamResponse.ok) {
...
// 获取代理基础地址（支持 local/sandbox 两种访问目标）。
...
/**
 * 获取内部 API 使用的代理基础地址（如定时任务）。
 * 与 LLM 代理地址不同，这里始终返回本地地址。
 */
