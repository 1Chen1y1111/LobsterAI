...
export async function downloadUpdate(url: string, onProgress: (progress: AppUpdateDownloadProgress) => void): Promise<string> {
  if (activeDownloadController) {
...
async function installMacDmg(dmgPath: string): Promise<void> {
  ...
  try {
    ...
    const mountOutput = await execAsync(`hdiutil attach ${shellEscape(dmgPath)} -nobrowse -noautoopen -noverify`, 60_000)
    ...
    try {
      ...
      await execAsync(`rm -rf ${shellEscape(targetApp)} && cp -R ${shellEscape(sourceApp)} ${shellEscape(targetApp)}`, 300_000)
      ...
      try {
        ...
        throw new Error(`Installation failed: insufficient permissions. ${adminError instanceof Error ? adminError.message : ''}`)
      }
    }
