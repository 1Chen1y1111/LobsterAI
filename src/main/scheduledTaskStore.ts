...
export class ScheduledTaskStore {
  ...
  updateTask(id: string, input: Partial<ScheduledTaskInput>): ScheduledTask | null {
    ...
    if (input.schedule !== undefined || input.enabled !== undefined) {
      nextRunAtMs = enabled ? this.calculateNextRunTime(schedule, existing.state.lastRunAtMs) : null
    }
  ...
  markTaskCompleted(id: string, success: boolean, durationMs: number, error: string | null, schedule: Schedule): void {
    const now = Date.now()
    ...
    this.db.run(
      ...
      WHERE id = ?
    `,
      [now, success ? 'success' : 'error', error, durationMs, consecutiveErrors, nextRunAtMs, new Date().toISOString(), id]
    )
  ...
  listRuns(taskId: string, limit: number = 50, offset: number = 0): ScheduledTaskRun[] {
    const rows = this.getAll<RunRow>('SELECT * FROM scheduled_task_runs WHERE task_id = ? ORDER BY started_at DESC LIMIT ? OFFSET ?', [
      taskId,
      limit,
      offset
    ])
  ...
  countRuns(taskId: string): number {
    const row = this.getOne<{ 'COUNT(*)': number }>('SELECT COUNT(*) FROM scheduled_task_runs WHERE task_id = ?', [taskId])
    return row?.['COUNT(*)'] ?? 0
  }

  pruneRuns(taskId: string, keepCount: number = 100): void {
    // Get IDs of the runs to keep
    const keepRows = this.getAll<{ id: string }>('SELECT id FROM scheduled_task_runs WHERE task_id = ? ORDER BY started_at DESC LIMIT ?', [
      taskId,
      keepCount
    ])
    ...
    this.db.run(`DELETE FROM scheduled_task_runs WHERE task_id = ? AND id NOT IN (${placeholders})`, [taskId, ...keepIds])
    this.saveDb()
  }
